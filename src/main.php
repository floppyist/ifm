...
f00bar;
    ...
    private function dispatch() {
        ...
        case "getDetailFileInfo": 
            return $this->getDetailItemInformation($_REQUEST["dir"], $_REQUEST["filenames"]);
        }
    ...
    /*
     * Removed heavy syscall requests to reduce loading time a lot
     */
    private function getBasicItemInformation($name) {
        $item = [];
    ...
    /*
     * This will used by the frontend to gather only heavy file informations for specific (visible) elements
     */
    private function getDetailItemInformation($dir, $filenames) {
        $result = [];
        $userCache = [];
        $groupCache = [];

        if (!isset($filenames)) return;
    ...
    private function searchItemsRecursive($pattern, $dir="") {
        ...
        foreach (glob($this->pathCombine($dir, $pattern)) as $result)
            array_push($items, $this->getBasicItemInformation($result));
    ...
    private function createDir($d) {
        ...
        if (@mkdir($d['dirname']))
            return ["status" => "OK", "message" => $this->l('folder_create_success'), "fileData" => $this->getBasicItemInformation($d['dirname'])];
        else
    ...
    private function saveFile($d) {
        ...
        if (isset($d['content'])) {
            if (@file_put_contents($d['filename'], $d['content']) !== false)
                return ["status" => "OK", "message" => $this->l('file_save_success'), "fileData" => $this->getBasicItemInformation($d['filename'])];
            else
    ...
    private function editFile(array $d) {
        ...
        if (isset($d['content'])) {
            if (@file_put_contents($rename, $d['content']) !== false)
                return ["status" => "OK", "message" => $this->l('file_save_success'), "fileData" => $this->getBasicItemInformation($rename)];
            else
                throw new Exception($this->l('file_save_error'));
        }
